    ...
    if (!empty($arResult['ITEMS']))
    {
        ...
        if ($bGetProperty)
        {
            ...
            */
            ...
            $arFilter = Array("ID"=>array_keys($arResult["ITEMS"]));
            $iterator = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
            while($ob = $iterator->GetNextElement())
            {
                $arFields = $ob->GetFields();
                $arProperties = $ob->GetProperties();
                $arResult["ITEMS"][$arFields["ID"]]["PROPERTIES"] = $arProperties;
            }
        }
    }
