  ...
  function ajaxForm (obForm) {
    obForm.onsubmit = async (e) => {
      ...
      for (let [name, value] of formData) {
        let input = obForm.querySelector('input[name=\'' + name + '\']')
        if (!input) {
          continue
        }
        let dataName = input.getAttribute('data-name')
        formData.set(dataName, value)
      }

      let response = await fetch('/local/ajax/form.php', {
        method: 'POST',
        body: formData,
      })

      obForm.querySelector('.error-msg').innerHTML = ''
      let result = await response.json()
      if (!result.success) {
        obForm.querySelector('.error-msg').innerHTML = result.error.join('<br>')
        return
      }
    ...
    success: function (response) {
      ...
      if (result == 1) {
        ...
        if ($('.mu-save-mark[data-item="' + id + '"]').length) {
          $('.mu-save-mark[data-item="' + id + '"] svg use').
            attr('xlink:href', '#markFilled')
        }
      }
      else if (result == -1) {
        if ($('.favor[data-item="' + id + '"]').length) {
